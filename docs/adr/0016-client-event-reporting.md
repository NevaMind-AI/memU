...
| `cli_uninstalled` | `report uninstall` |
| `cli_error` | an exception the CLI caught — §5 |
| `cli_events_dropped` | the spool hit its cap and discarded events |

The `cli_` prefix marks what the client observed about *itself* — its own install, its own
uncaught exception, its own spool overflowing — against the `core_action_` pair, which reports
on memU's actual work. That is why the agent-reported error sits with the latter: it is a claim
about a retrieval or a bridging run, not about the binary.
...
**`core_action_failed` — model-judged.** Invoked deliberately by the agent when it decides
...
It is named as the failure counterpart to `core_action_completed` and carries the same
discriminator, `properties.action_name`, so a consumer slices both events the same way. That
field is filled from `--stage` and is the *same string*; the flag, the prose, and every
agent-facing instruction still say **stage** and only stage — the second name exists at the
envelope, not in anything a user or an agent types. The two vocabularies do not yet line up
(`stage` says `remember`/`retrieve` where a core action says `memory_update`/`memory_search`),
so the events do not join on this field as they stand. That is deliberate: `stage` also spans
`install`, `uninstall`, and `other`, which name no core action at all, and translating the two
that overlap would leave the rest lying.
