"""
Test MiniMax integration with MemU's full workflow.

Tests:
1. Conversation memorization using MiniMax
2. RAG-based retrieval using MiniMax embeddings
3. LLM-based retrieval using MiniMax
...
async def _test_memorize(service, file_path, output_data):
    ...
    print("\n[MINIMAX] Test 1: Memorizing conversation...")
    memory = await service.memorize(
        resource_url=file_path, modality="conversation", user={"user_id": "minimax_test_user"}
    )
...
async def _test_retrieve(service, queries, method, test_num, output_data):
    ...
    print(f"\n[MINIMAX] Test {test_num}: {method.upper()}-based retrieval...")
    service.retrieve_config.method = method
    result = await service.retrieve(queries=queries, where={"user_id": "minimax_test_user"})
...
async def test_minimax_full_workflow():
    """Test MiniMax integration with full MemU workflow."""
    ...
    print("\n" + "=" * 60)
    print("[MINIMAX] Starting full workflow test...")
    print("=" * 60)
    ...
    print("\n[MINIMAX] Test 4: List memory items...")
    items_result = await service.list_memory_items(where={"user_id": "minimax_test_user"})
    ...
    print("\n[MINIMAX] Test 5: List memory categories...")
    cats_result = await service.list_memory_categories(where={"user_id": "minimax_test_user"})
    ...
    output_file = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "examples", "output", "minimax_test_output.json")
    )
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(output_data, f, indent=2, default=str)
    print(f"\n[MINIMAX] Output saved to: {output_file}")

    print("\n" + "=" * 60)
    print("[MINIMAX] All tests completed!")
    print("=" * 60)


if __name__ == "__main__":
    asyncio.run(test_minimax_full_workflow())
