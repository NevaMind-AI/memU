...
async def _test_memorize(service, file_path, user_id, output_data):
    ...
    memory = await service.memorize(resource_url=file_path, modality="conversation", user={"user_id": user_id})
...
async def test_gemini_full_workflow():
    ...
    file_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "examples", "resources", "conversations", "conv1.json")
    )
