...
class TestNoDuplicateMethods:
    ...
    def test_memorize_has_category_embedding_text(self):
        """Test that MemorizeMixin has _category_embedding_text."""
        import inspect

        from memu.app.memorize import MemorizeMixin

        # Note: MemorizeMixin has its own _category_embedding_text method
        # (not inherited from WorkflowMixin due to signature incompatibility)
        own_methods = set()
        for name, _method in inspect.getmembers(MemorizeMixin, predicate=inspect.isfunction):
            own_methods.add(name)
