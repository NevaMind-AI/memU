...
- ``skill/**``  : reusable skills. When the caller supplies a synthesized
  ...
  map is supplied (``synthesize=False``), it falls back to a deterministic,
  LLM-free bypass that breaks out the extracted skill-type memory items.
...
class MemoryFileExporter:
    ...
    def export(
        ...
        *,
        ...
        and its root ``SKILL.md`` index are sourced from it. When ``skills`` is
        ``None`` the exporter falls back to a deterministic, LLM-free bypass that
        derives the tree from the extracted skill-type memory items.
        ...
        # The skill/ tree is sourced from the synthesized ``skills`` map when the
        # caller supplies one (see MemorySynthesizer); otherwise it falls back to a
        # deterministic, LLM-free bypass over the extracted skill-type memory items.
        skill_map = skills if skills is not None else self._skill_bypass(items)
        skill_artifacts = {
