...
Each artifact is a different aggregation of what the agent has stored. The two
root indexes (``INDEX.md`` / ``MEMORY.md``) each point at a sibling directory of
payloads (``resource/`` / ``memory/``):
...
class MemoryFileExporter:
    ...
    def build_synthesis_descriptions(resources: list[Resource], items: list[Entry]) -> list[FileDescription]:
        ...
        for resource in sorted(resources, key=lambda r: (r.url, r.id)):
            res_items = sorted(
                ...
                key=lambda i: (i.entry_type, i.created_at, i.id),
            )
            parts = [
                f"[{item.entry_type}] {' '.join((item.text or '').split())}"
    ...
    def _prune_empty_dirs(self, directory: pathlib.Path) -> None:
        """Remove now-empty directories created for nested artifacts (e.g. memory/)."""
    ...
    def read_existing(self) -> ExistingArtifacts:
        """Load the prior MEMORY artifact as a single bundle for merging."""
        return ExistingArtifacts(memory_body=self.read_memory_body())
