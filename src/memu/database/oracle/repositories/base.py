...
class OracleRepoBase:
    ...
    def _normalize_embedding(self, embedding: Any) -> list[float] | None:
        if embedding is None:
            return None
        # Oracle JSON handling should natively return lists or dicts depending on driver/sqlalchemy
        if isinstance(embedding, list):
            try:
                return [float(x) for x in embedding]
            except Exception:
                logger.debug("Could not normalize list embedding %s", embedding)
                return None
