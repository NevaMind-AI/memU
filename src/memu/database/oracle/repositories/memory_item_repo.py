...
from memu.database.oracle.repositories.base import OracleRepoBase
from memu.database.oracle.session import SessionManager
from memu.database.state import DatabaseState


class OracleMemoryItemRepo(OracleRepoBase):
    def __init__(
        self,
        *,
        state: DatabaseState,
    ...
    def vector_search_items(
        ...
        # For Oracle initial implementation, we fallback to local vector search
        # as native in-db search requires specific version/setup (23ai).
        # Same behavior as Postgres when _use_vector is False.
        if not self._use_vector:
            return self._vector_search_local(query_vec, top_k, where=where)

        # TODO: Implement native Oracle Vector search if available
        # logic would go here if use_vector is True
        return self._vector_search_local(query_vec, top_k, where=where)
