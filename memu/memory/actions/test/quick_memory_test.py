...
class LLMLoggingWrapper:
    ...
    def __init__(self, llm_client: BaseLLMClient, log_dir: str, character_name: str):
        ...
        self.main_log_file = self.character_log_dir / f"{character_name}_llm_interactions.log"
    ...
    def chat_completion(self, messages, model=None, temperature=0.7, max_tokens=1000, **kwargs):
        ...
        # Log the input
        self._log_interaction(call_id, "INPUT", {
            "messages": messages,
            "model": model,
        ...
        try:
            ...
            # Log the output
            self._log_interaction(call_id, "OUTPUT", {
            ...
            # Log the error
            self._log_interaction(call_id, "ERROR", {
                "error": str(e),
                "duration_seconds": round(duration, 3)
            })
    ...
    def _serialize_any_object(self, obj):
        ...
        if hasattr(obj, '__dict__'):
            try:
                return {str(k): self._serialize_any_object(v) for k, v in obj.__dict__.items()}
            except Exception:
                pass
    ...
    def _log_interaction(self, call_id: str, interaction_type: str, data: dict):
        ...
        interaction_file = self.character_log_dir / f"{call_id}_{interaction_type.lower()}.json"
        ...
        try:
            ...
            safe_entry = {
                ...
                "data": {
                    ...
                    "success": str(data.get("success")) if isinstance(data, dict) else "unknown"
                }
            }
            ...
            try:
                ...
                with open(interaction_file.with_suffix('.txt'), 'w', encoding='utf-8') as f:
...
def create_llm_client(log_dir: str = None, character_name: str = "default"):
    ...
    if provider == 'azure':
        ...
        client = AzureOpenAIClient(api_key=api_key, azure_endpoint=endpoint, deployment_name=deployment, api_version=api_version)
...
class LocomoDataLoader:
    ...
    def get_session_conversations(self, sample, session_num=1):
        ...
        if conversations and len(conversations) > 0:
            ...
            print(f"📊 Session {session_num} structure - first conversation keys: {list(first_conv.keys()) if isinstance(first_conv, dict) else 'Not a dict'}")
...
def quick_memory_test():
        ...
        for key in conv_keys:
            if key.startswith('session_') and not key.endswith('_date_time'):
                ...
                if first_session_data:
                    print(f"   First session ({key}) has {len(first_session_data)} items")
    ...
    try:
        ...
        try:
            llm_client = create_llm_client(log_dir=llm_logs_dir, character_name=test_character)
        ...
        for session_idx, session_num in enumerate(test_sessions, 1):
            ...
            if not character_dialogues:
                ...
                print(f"   Available speakers in session {session_num}: {list(available_speakers)}")
                continue
            ...
            for i, dialog in enumerate(conversations[:3], 1):
                ...
                print(f"  {i}. {speaker_emoji} {dialog['speaker']}: {dialog['text'][:70]}...")
            ...
            try:
                ...
                for i, dialog in enumerate(conversations[:5]):  # Check first 5
                    ...
                    print(f"   Dialog {i+1}: speaker='{speaker}', text='{text[:50]}{'...' if len(text) > 50 else ''}'")
                ...
                for dialog in conversations:
                    ...
                    speaker = dialog.get('speaker', '').strip() if dialog.get('speaker') else ''
                ...
                print(f"   Processed dialogues for memory agent: {len(conversation_list)}")
            ...
            if os.path.exists(memory_file):
                ...
                if memory_lines:
                    ...
                    for i, line in enumerate(memory_lines[:3], 1):
                        ...
                        item_content = line[line.find(']') + 1:].strip()[:60] if ']' in line else line[:60]
        ...
        if character_log_dir.exists():
            ...
            output_files = len([f for f in log_files if f.name.endswith('_output.json')])
