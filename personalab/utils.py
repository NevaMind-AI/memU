...
def simulate_ai_response(memory: Memory, user_message: str, conversation_history: Optional[List[Dict[str, Any]]] = None) -> str:
    """
    ...
    if any(word in user_msg_lower for word in ["工作", "就业", "职业", "面试", "求职"]):
        tech_interests = any(keyword in ' '.join(events + insights).lower() 
    ...
    # 默认响应
    if profile:
        return f"我理解您的问题。基于我对您的了解，我建议从您感兴趣的领域开始深入。有什么具体的方面需要我详细解释吗？"
    else:
...
def extract_events_from_conversation(messages: List[Dict[str, str]]) -> List[str]:
    ...
    # 事件关键词
    event_keywords = [
        "学习", "问题", "项目", "工作", "购买", "计划", "完成", "开始", "决定",
        "使用", "尝试", "创建", "开发", "研究", "分析", "设计", "实现"
    ]
...
def extract_insights_from_conversation(messages: List[Dict[str, str]], current_insights: List[str] = None) -> List[str]:
    """
    从对话消息中提取用户洞察和特征
    ...
    # 技术兴趣分析
    technical_keywords = ["代码", "编程", "算法", "api", "数据库", "机器学习", "ai", "python", "java", "javascript"]
    ...
    if any(keyword in all_user_text for keyword in ["机器学习", "ai", "算法"]) and "用户对机器学习领域感兴趣" not in current_insights:
        insights.append("用户对机器学习领域感兴趣")
    
    if any(keyword in all_user_text for keyword in ["web", "网站", "前端", "后端"]) and "用户对Web开发感兴趣" not in current_insights:
...
def create_memory_manager(db_path: str = "memory.db"):
    """创建Memory管理器"""
    return MemoryClient(db_path=db_path)


def create_conversation_manager(db_path: str = "conversations.db", enable_embeddings: bool = True):
    """创建对话管理器"""
    return ConversationManager(db_path=db_path, enable_embeddings=enable_embeddings)
...
def get_conversation_context(conversation_manager, agent_id: str, query: str, limit: int = 2) -> str:
    """获取相关的历史对话上下文"""
    try:
        # 搜索相关对话
...
def build_system_prompt(memory_manager, conversation_manager, agent_id: str, user_message: str) -> str:
    """构建包含记忆的系统提示"""
