    ...
    def _init_database(self):
        ...
        with psycopg2.connect(self.connection_string) as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("CREATE INDEX IF NOT EXISTS idx_conversations_agent_id ON conversations(agent_id)")
                ...
                cur.execute("CREATE INDEX IF NOT EXISTS idx_messages_conversation_id ON conversation_messages(conversation_id)")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_messages_role ON conversation_messages(role)")
                
                cur.execute("CREATE INDEX IF NOT EXISTS idx_vectors_agent_id ON embedding_vectors(agent_id)")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_vectors_source_type ON embedding_vectors(source_type)")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_vectors_content_hash ON embedding_vectors(content_hash)")
        ...
        try:
            with psycopg2.connect(self.connection_string) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute("DELETE FROM conversation_messages WHERE conversation_id = %s", 
                               [conversation.conversation_id])
            ...
            with psycopg2.connect(self.connection_string) as conn:
                with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                    ...
                    cur.execute("""
                        ...
                        LIMIT %s
                    """, (vector_param, agent_id, vector_param, similarity_threshold, vector_param, limit))
