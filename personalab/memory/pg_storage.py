...
class PostgreSQLMemoryDB:
    ...
    def _init_database(self):
        ...
        with psycopg2.connect(self.connection_string) as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("CREATE INDEX IF NOT EXISTS idx_memories_agent_id ON memories(agent_id)")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_memories_user_id ON memories(user_id)")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_memories_agent_user ON memories(agent_id, user_id)")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_memories_updated_at ON memories(updated_at)")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_memories_schema_version ON memories(schema_version)")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_memory_contents_memory_type ON memory_contents(memory_id, content_type)")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_memory_contents_hash ON memory_contents(content_hash)")
                ...
                with conn.cursor() as cur:
                    ...
                    cur.execute("""
                        ...
                        (memory_id, agent_id, user_id, created_at, updated_at, tom_metadata, 
                         ...
                         event_content, tom_content)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                        ON CONFLICT (memory_id) DO UPDATE SET
                        updated_at = EXCLUDED.updated_at,
                        tom_metadata = EXCLUDED.tom_metadata,
                        ...
                        tom_content = EXCLUDED.tom_content
                    """, (
                        memory.memory_id,
                        memory.agent_id,
                        memory.user_id,
            ...
            with psycopg2.connect(self.connection_string) as conn:
                with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                    ...
                    if memory_row['tom_metadata']:
                        memory.mind_metadata = memory_row['tom_metadata']
    ...
    def save_memory_embedding(
        ...
        Args:
            memory_id: Memory ID
            content_type: Type of content ('profile', 'event', 'tom')
            ...
            with psycopg2.connect(self.connection_string) as conn:
                with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                    ...
                    return {
                        ...
                        'last_updated': stats_row['last_updated'].isoformat() if stats_row['last_updated'] else None,
