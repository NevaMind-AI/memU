...
memU 不会把智能体学到的一切压扁进一个巨大的 prompt 或一团不透明的向量里，而是像你管理电脑一样组织记忆——一个可浏览、能自组织的文件系统：

- **资源（Resources）** 是智能体摄入的*原始文件*：对话、文档、URL、图片、音频、视频和日志
- **记忆项（Memory items）** 是*文件*——从这些来源中提取出的、带类型的结构化笔记
- **类别（Categories）** 是*文件夹*——它们把相关记忆归到一起，并维护持续演进的摘要
- **智能体就是操作系统**——它通过 `memorize()` 写入新来源，通过 `retrieve()` 按需读取限定范围、经过排序的上下文
