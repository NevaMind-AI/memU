...
에이전트가 학습한 모든 것을 거대한 프롬프트나 불투명한 벡터 덩어리로 짓눌러 넣는 대신, memU는 컴퓨터를 정리하듯 메모리를 정리합니다 — 탐색 가능하고 스스로 조직화되는 파일 시스템으로.

- **리소스(Resources)** 는 에이전트가 받아들이는 *원본 파일*입니다: 대화, 문서, URL, 이미지, 오디오, 비디오, 로그
- **메모리 아이템(Memory items)** 은 *파일*입니다 — 그 소스에서 추출한 타입이 지정된 구조화 노트
- **카테고리(Categories)** 는 *폴더*입니다 — 관련 메모리를 묶고 끊임없이 진화하는 요약을 유지합니다
- **에이전트는 운영체제입니다** — `memorize()`로 새 소스를 쓰고, `retrieve()`로 필요할 때 범위가 지정되고 순위가 매겨진 컨텍스트를 읽습니다
