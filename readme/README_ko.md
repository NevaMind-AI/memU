...
> [!WARNING]
> 🚧 **대규모 개편 진행 중** — memU는 대규모 리워크 중이며 API, CLI 명령어, 문서가 예고 없이 변경될 수 있습니다. **2026년 7월 15일**경 안정화될 예정입니다.
...
| 레코드 | 파일 시스템 역할 | 사용 목적 |
|--------|------------------|---------|
| `RecallFile` | **폴더** — 관련 메모리를 묶고(또는 스킬을 담고) 주제 수준 요약을 유지; `track` 필드가 `memory` 또는 `skill`을 표시 | 폭넓은 질의를 위해 간결한 컨텍스트를 로드 |
