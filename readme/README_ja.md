...
エージェントが学んだすべてを巨大なプロンプトや不透明なベクトルの塊に押し込むのではなく、memU はコンピュータを整理するのと同じように記憶を整理します——ナビゲート可能で自己組織化するファイルシステムとして。

- **リソース（Resources）** はエージェントが取り込む*生のファイル*：チャット、ドキュメント、URL、画像、音声、動画、ログ
- **メモリアイテム（Memory items）** は*ファイル*——それらのソースから抽出された、型付きで構造化されたノート
- **カテゴリ（Categories）** は*フォルダ*——関連する記憶をまとめ、進化し続けるサマリーを保持する
- **エージェントはオペレーティングシステム**——`memorize()` で新しいソースを書き込み、`retrieve()` で必要に応じてスコープ付き・ランク付けされたコンテキストを読み出す
